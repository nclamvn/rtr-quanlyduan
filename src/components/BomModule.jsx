...
import { BOM_CATEGORY_COLORS, BOM_LIFECYCLE_COLORS } from "../data/v2Data";
...
function BomTreeRow({
  ...
  const isWarning =
    item.lifecycleStatus === "EOL" || item.lifecycleStatus === "OBSOLETE" || item.lifecycleStatus === "NRND";
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (!selected) e.currentTarget.style.background = "var(--border-a20)";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      {/* Description */}
      <span
        title={lang === "vi" && item.descriptionVi ? item.descriptionVi : item.description}
        style={{
          fontSize: 13,
          color: isAssembly ? "var(--text-primary)" : "var(--text-secondary)",
      ...
      {isWarning && (
        <Badge
          ...
          icon={item.lifecycleStatus === "OBSOLETE" ? ShieldAlert : AlertTriangle}
        />
      )}
      ...
      {canViewCost && (
        <span
          style={{
            fontSize: 13,
            fontFamily: mono,
            color: isAssembly ? "#10B981" : "var(--text-muted)",
            width: 70,
            textAlign: "right",
            flexShrink: 0,
            fontWeight: isAssembly ? 700 : 400,
...
export default function BomModule({ lang, t: _t, project, perm }) {
  ...
  const costSummary = useMemo(() => {
    ...
    const supplierCosts = {};
    projectBom
      .filter((b) => b.level === 2 && b.supplierId)
      .forEach((b) => {
    ...
    const bySupplier = Object.entries(supplierCosts)
    ...
    projectBom
      .filter((b) => b.level === 2)
      .forEach((b) => {
        lifecycleCounts[b.lifecycleStatus] = (lifecycleCounts[b.lifecycleStatus] || 0) + 1;
      });
    const byLifecycle = Object.entries(lifecycleCounts).map(([k, v]) => ({
      name: k,
      value: v,
      color: BOM_LIFECYCLE_COLORS[k] || "#6B7280",
    }));
  ...
  const flattenTree = (items, depth = 0) => {
    ...
    for (const item of items) {
      ...
      if (search && item.level === 2) {
        ...
        const match =
          normalizeVN(item.partNumber).includes(s) ||
          normalizeVN(item.description).includes(s) ||
          normalizeVN(item.descriptionVi || "").includes(s);
        if (!match) continue;
      }
  ...
  const visibleRows = useMemo(
    () => flattenTree(rootItems),
    [rootItems, expandedNodes, filterCategory, filterLifecycle, search, childrenMap],
  );
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, color: "var(--text-dim)" }}>
            {canViewCost && (
              <>
                <DollarSign size={10} style={{ display: "inline", verticalAlign: "middle" }} /> $
          ...
          {subTabs.map((st) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 600,
                color: subTab === st.id ? "#60A5FA" : "var(--text-dim)",
                cursor: "pointer",
              }}
            >
              {st.label}
          ...
          {costSummary.eolCount > 0 && (
            ...
            >
              ...
              <span style={{ fontSize: 12, color: "#F97316", fontWeight: 600 }}>
                {costSummary.eolCount} EOL/Obsolete
              </span>
            </div>
          )}
          ...
          >
            ...
            >
              <div style={{ position: "relative", flex: 1, minWidth: 120 }}>
                ...
                <input
                  placeholder={lang === "vi" ? "Tìm part..." : "Search parts..."}
                  value={search}
              ...
              >
                ...
                {["MECHANICAL", "ELECTRICAL", "CONSUMABLE"].map((c) => (
              ...
              >
                ...
                {["ACTIVE", "NRND", "EOL", "OBSOLETE"].map((l) => (
            ...
            <div style={{ maxHeight: 420, overflowY: "auto" }}>
              {projectBom.length === 0 && (
                <EmptyState
                  icon={EMPTY_MESSAGES[lang].bom.icon}
                  title={EMPTY_MESSAGES[lang].bom.title}
                  description={EMPTY_MESSAGES[lang].bom.desc}
                />
              )}
          ...
          {selectedItem && (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontSize: 13, fontFamily: mono, color: "var(--text-dim)" }}>
                    {selectedItem.partNumber}
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", marginTop: 2 }}>
                    {lang === "vi" && selectedItem.descriptionVi
                      ? selectedItem.descriptionVi
                      : selectedItem.description}
                  </div>
                </div>
                <Badge
                  label={selectedItem.lifecycleStatus}
                  color={BOM_LIFECYCLE_COLORS[selectedItem.lifecycleStatus]}
                />
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <div style={{ background: "var(--bg-input)", borderRadius: 4, padding: "6px 8px" }}>
                  ...
                  >
                    {lang === "vi" ? "Loại" : "Category"}
                  </div>
                  <div
                    style={{
                      fontSize: 13,
                      color: BOM_CATEGORY_COLORS[selectedItem.category],
                      fontWeight: 600,
                      display: "flex",
                      alignItems: "center",
                      gap: 4,
                  ...
                  >
                    {(() => {
                      const I = CATEGORY_ICONS[selectedItem.category];
                ...
                <div style={{ background: "var(--bg-input)", borderRadius: 4, padding: "6px 8px" }}>
                  ...
                  <div style={{ fontSize: 13, color: "var(--text-primary)", fontFamily: mono, marginTop: 2 }}>
                ...
                {canViewCost && (
                  <>
                    <div style={{ background: "var(--bg-input)", borderRadius: 4, padding: "6px 8px" }}>
                      ...
                      >
                        {lang === "vi" ? "Đơn giá" : "Unit Cost"}
                      </div>
                      <div style={{ fontSize: 13, color: "#10B981", fontFamily: mono, marginTop: 2 }}>
                        {selectedItem.unitCost != null ? `$${selectedItem.unitCost}` : "—"}
                      </div>
                    </div>
                    <div style={{ background: "var(--bg-input)", borderRadius: 4, padding: "6px 8px" }}>
                      <div
                      ...
                      <div style={{ fontSize: 13, color: "#10B981", fontFamily: mono, fontWeight: 700, marginTop: 2 }}>
                        ${(selectedItem.totalCost || 0).toLocaleString()}
                      </div>
                    </div>
                  </>
                )}
                ...
                <div style={{ background: "var(--bg-input)", borderRadius: 4, padding: "8px 10px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                      {lang === "vi" ? selectedSupplier.nameVi : selectedSupplier.name}
                    </span>
                  </div>
                  <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                    <span style={{ fontSize: 11, color: "var(--text-dim)" }}>
                      LT: <span style={{ color: "#F59E0B", fontFamily: mono }}>{selectedItem.leadTimeDays}d</span>
                    </span>
                    <span style={{ fontSize: 11, color: "var(--text-dim)" }}>
                      {lang === "vi" ? "Chất lượng" : "Quality"}:{" "}
                      <span style={{ color: "#10B981" }}>{"★".repeat(Math.round(selectedSupplier.qualityRating))}</span>
                    </span>
                  </div>
                </div>
              )}
              ...
              {selectedItem.designator && (
                <div style={{ fontSize: 12, color: "var(--text-dim)" }}>
                  ...
                  <span style={{ fontFamily: mono, color: "var(--text-muted)" }}>{selectedItem.designator}</span>
                </div>
              )}
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 16 }}>
          <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
            ...
            >
              ...
              >
                {lang === "vi" ? "Tổng chi phí BOM" : "Total BOM Cost"}
              </div>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#10B981", fontFamily: mono, marginTop: 4 }}>
                ${costSummary.total.toLocaleString()}
              </div>
            </div>
            <div
            ...
            >
              ...
              >
                {lang === "vi" ? "Linh kiện" : "Components"}
              </div>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#3B82F6", fontFamily: mono, marginTop: 4 }}>
                {costSummary.totalParts}
              </div>
              <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 2 }}>
                {costSummary.assemblies} {lang === "vi" ? "bộ lắp ráp" : "assemblies"}
              </div>
            </div>
          </div>

          <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>
            {lang === "vi" ? "Chi phí theo loại" : "Cost by Category"}
          </div>
            ...
            const bomTooltip = ({ active, payload }) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontFamily: mono, color: d.payload?.color || "#3B82F6" }}>
                    ${(d.value || d.payload?.value || 0).toLocaleString()}
                  </div>
                </div>
              );
            };
            return (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 16 }}>
                ...
                <div style={{ minWidth: 0, minHeight: 200 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>
                    {lang === "vi" ? "Chi phí theo cụm lắp ráp" : "Cost by Sub-Assembly"}
                  </div>
                  <SafeResponsiveContainer width="100%" height={200} minWidth={0} minHeight={0}>
                    <PieChart>
                      <Pie
                        data={costSummary.bySubAssy}
                        cx="50%"
                        cy="50%"
                        innerRadius={45}
                        outerRadius={75}
                        ...
                        style={{ fontSize: 10, fontFamily: sans }}
                      >
                        {costSummary.bySubAssy.map((_, i) => (
                ...
                <div style={{ minWidth: 0, minHeight: 200 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>
                    {lang === "vi" ? "Chi phí theo nhà cung cấp" : "Cost by Supplier"}
                  </div>
                  <SafeResponsiveContainer width="100%" height={200} minWidth={0} minHeight={0}>
                    <BarChart data={costSummary.bySupplier} layout="vertical" margin={{ left: 5 }}>
                      ...
                      <XAxis
                        type="number"
                        tick={{ fill: tickColor, fontSize: 10, fontFamily: mono }}
                        axisLine={false}
                        tickLine={false}
                        tickFormatter={(v) => `$${v}`}
                      />
                      <YAxis
                        type="category"
                        dataKey="name"
                        tick={{ fill: tickColor, fontSize: 11, fontFamily: mono }}
                        axisLine={false}
                        tickLine={false}
                        width={40}
                      />
                      ...
                      <Bar dataKey="value" name={lang === "vi" ? "Chi phí" : "Cost"} radius={[0, 4, 4, 0]} barSize={18}>
                        {costSummary.bySupplier.map((_, i) => (
                          <Cell key={i} fill={COLORS[i % COLORS.length]} />
                        ))}
                      </Bar>
                    </BarChart>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>
                    {lang === "vi" ? "Rủi ro vòng đời linh kiện" : "Component Lifecycle Risk"}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                    <SafeResponsiveContainer width={160} height={140} minWidth={0} minHeight={0}>
                      <PieChart>
                        <Pie
                          data={costSummary.byLifecycle}
                          cx="50%"
                          cy="50%"
                          innerRadius={35}
                          outerRadius={55}
                    ...
                    <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                      {costSummary.byLifecycle.map((d, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                          ...
                          <span style={{ fontFamily: mono, fontSize: 12, fontWeight: 700, color: d.color }}>
                            {d.value}
                          </span>
                        </div>
                      ))}
      ...
      {subTab === "cost" && !canViewCost && (
        ...
        >
          ...
          <div style={{ fontSize: 14, color: "var(--text-dim)" }}>
            {lang === "vi" ? "Bạn không có quyền xem chi phí" : "You don't have permission to view costs"}
          </div>
        </div>
      )}
    </div>
