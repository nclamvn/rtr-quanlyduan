...
import { Download, FileSpreadsheet, FileText, Presentation, X, Check, Loader2 } from "lucide-react";
...
export function exportBomExcel(bomParts, suppliers, lang) {
  ...
  exportToExcel(
    [
      ...
      {
        name: vi ? "Tóm tắt chi phí" : "Cost Summary",
        data: summaryData,
        columns: [
...
export function ProjectSummaryReport({
  ...
  issues,
  lang,
  gateConfig: _gateConfig,
  gateChecks: _gateChecks,
  bomParts,
  flightTests: flightTestsProp,
}) {
  const vi = lang === "vi";
    ...
    <div style={{ width: 794, padding: 40, background: "#060A0F", color: "#E2E8F0", fontFamily: sans, fontSize: 11 }}>
      ...
      >
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, fontWeight: 700 }}>
            {vi ? "Báo cáo Trạng thái Dự án" : "Project Status Report"}
          </div>
          <div style={{ fontSize: 8, color: "#64748B" }}>
            {new Date().toLocaleDateString(vi ? "vi-VN" : "en-US")} | {project?.name}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, marginBottom: 20 }}>
        {[
          ...
          >
            <div style={{ fontSize: 7, color: "#475569", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              {m.label}
            </div>
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid #1E2A3A" }}>
              {[
                ...
                <th key={h} style={{ padding: "4px 6px", textAlign: "left", color: "#475569", fontWeight: 700 }}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {openIssues.slice(0, 10).map((i) => (
              <tr key={i.id} style={{ borderBottom: "1px solid #0F1419" }}>
                ...
                <td
                  style={{
                    padding: "3px 6px",
                    color: i.sev === "CRITICAL" ? "#EF4444" : i.sev === "HIGH" ? "#F59E0B" : "#94A3B8",
                    fontWeight: 700,
                  }}
                >
                  {i.sev}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      <div style={{ ...slideStyle, justifyContent: "center", alignItems: "center", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12, color: "#64748B" }}>
          {vi ? "Báo cáo Trạng thái" : "Status Update"} |{" "}
          {new Date().toLocaleDateString(vi ? "vi-VN" : "en-US", { month: "long", year: "numeric" })}
        </div>
        <div
          style={{
            marginTop: 24,
      ...
      <div style={slideStyle}>
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 24 }}>
          {vi ? "Tổng quan Trạng thái" : "Status Overview"}
        </div>
                ...
                criticalIssues.length > 1
                  ...
                  : criticalIssues.length > 0
                    ? vi
                      ...
                      : "CAUTION"
                    : "OK",
              color: criticalIssues.length > 1 ? "#EF4444" : criticalIssues.length > 0 ? "#F59E0B" : "#10B981",
            },
          ].map((m, i) => (
            <div
              key={i}
      ...
      <div style={slideStyle}>
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 24 }}>
          {vi ? "Rủi ro & Blockers" : "Key Risks & Blockers"}
        </div>
          ...
          {cascadeIssues.slice(0, 4).map((i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 6,
                padding: "12px 16px",
                borderLeft: `3px solid ${i.sev === "CRITICAL" ? "#EF4444" : "#F59E0B"}`,
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#E2E8F0" }}>
                  {i.id}: {vi && i.titleVi ? i.titleVi : i.title}
                </span>
                <span style={{ fontSize: 9, color: i.sev === "CRITICAL" ? "#EF4444" : "#F59E0B", fontWeight: 700 }}>
                  {i.sev}
                </span>
              </div>
      ...
      <div style={slideStyle}>
              ...
              <div key={phase} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                <div
                  style={{
                    fontSize: 10,
                    fontWeight: 700,
                    color: isCurrent ? "#3B82F6" : isPast ? "#10B981" : "#475569",
                    marginTop: 4,
                  }}
                >
                  {phase}
        ...
        {cascadeIssues.length > 0 && (
          ...
          >
            {vi
              ? "Cảnh báo: Cascade delays có thể ảnh hưởng mốc MP"
              : "Warning: Cascade delays may impact MP milestone"}
          </div>
        )}
      </div>
      ...
      <div style={slideStyle}>
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 24 }}>
          {vi ? "Hành Động Cần Thiết" : "Action Items"}
        </div>
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#E2E8F0" }}>
                  {vi && i.titleVi ? i.titleVi : i.title}
                </div>
                <div style={{ fontSize: 9, color: "#64748B" }}>
                  {vi ? "Chịu TN" : "Owner"}: {i.owner} | {vi ? "Hạn" : "Due"}: {i.due || "TBD"}
                </div>
              </div>
            </div>
          ))}
          {criticalIssues.length === 0 &&
            openIssues.slice(0, 3).map((i, idx) => (
              <div
                key={i.id}
                style={{
                  display: "flex",
  ...
  const handleExportPdf = async () => {
    ...
    try {
      if (type === "slides") {
        ...
        await generateSlidesPdf(
          [...slideEls],
          `${project?.name || "RtR"}_Slides_${new Date().toISOString().split("T")[0]}`,
        );
      } else {
