...
export default function IssueCharts({ issues, lang }) {
  ...
  const trendData = useMemo(
    () => computeTrendData(issues).map((d) => ({ ...d, name: isVi ? d.week : d.weekEn })),
    [issues, isVi],
  );
  ...
  const sevData = useMemo(() => {
    ...
    issues.forEach((i) => {
      if (counts[i.sev] !== undefined) counts[i.sev]++;
    });
    return Object.entries(counts)
      .filter(([, v]) => v > 0)
      .map(([k, v]) => ({
        name: isVi ? { CRITICAL: "Nghiêm trọng", HIGH: "Cao", MEDIUM: "Trung bình", LOW: "Thấp" }[k] : k,
        value: v,
        color: SEV_COLORS[k],
      }));
  }, [issues, isVi]);
  ...
  const statusData = useMemo(() => {
    ...
    return Object.entries(STATUS_COLORS)
      .map(([k, c]) => ({ name: labels[k] || k, value: counts[k] || 0, fill: c }))
      .filter((d) => d.value > 0);
  }, [issues, isVi]);
      ...
      <div className="chart-grid-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ textAlign: "center", fontSize: 13, color: "var(--text-dim)", fontFamily: sans, marginTop: -8 }}>
            {isVi ? "Tổng" : "Total"}:{" "}
            <span style={{ fontFamily: mono, fontWeight: 700, color: "var(--text-primary)" }}>{issues.length}</span>
          </div>
        </div>
          ...
          <SafeResponsiveContainer width="100%" height={220} minWidth={0} minHeight={0}>
            <BarChart data={statusData} layout="vertical" margin={{ left: 10 }}>
              ...
              <YAxis
                type="category"
                dataKey="name"
                tick={{ fill: tickColor, fontSize: 11, fontFamily: sans }}
                axisLine={{ stroke: gridColor }}
                tickLine={false}
                width={80}
              />
