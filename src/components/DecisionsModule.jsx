...
import { DECISION_STATUS_COLORS } from "../data/v2Data";
...
export default function DecisionsModule({ lang, t: _t, project, issues: _issues, onViewIssue, perm }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, color: "var(--text-dim)" }}>
            — {decisions.length} {lang === "vi" ? "quyết định" : "decisions"}
          </span>
          {readOnly && (
            <span
              style={{
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          >
            ...
            {["PROPOSED", "APPROVED", "SUPERSEDED", "REJECTED"].map((s) => (
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${isExpanded ? "#3B82F640" : "var(--border)"}`,
                borderRadius: 8,
                overflow: "hidden",
            ...
            >
              ...
              <div
                ...
                onMouseEnter={(e) => {
                  if (!isExpanded) e.currentTarget.style.background = "var(--border-a10)";
                }}
                onMouseLeave={(e) => {
              ...
              >
                ...
                />
                <span style={{ fontSize: 12, fontFamily: mono, color: "var(--text-dim)", flexShrink: 0 }}>
                  {dec.id}
                </span>
                ...
                <div style={{ padding: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      ...
                      <span style={{ fontSize: 12, color: "var(--text-faint)" }}>
                        {lang === "vi" ? "Người quyết định" : "Decision Maker"}:
                      </span>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                              ...
                              <span
                                style={{
                                  fontSize: 13,
                                  fontWeight: 700,
                                  color: isChosen ? "#10B981" : "var(--text-secondary)",
                                }}
                              >
                                {opt.label}
                              </span>
                            ...
                            <div style={{ display: "flex", gap: 12, paddingLeft: 18 }}>
                              <div style={{ display: "flex", alignItems: "flex-start", gap: 4 }}>
                                ...
                                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                                  {lang === "vi" && opt.prosVi ? opt.prosVi : opt.pros}
                                </span>
                              </div>
                              <div style={{ display: "flex", alignItems: "flex-start", gap: 4 }}>
                                <X size={9} color="#EF4444" style={{ marginTop: 2, flexShrink: 0 }} />
                                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                                  {lang === "vi" && opt.consVi ? opt.consVi : opt.cons}
                                </span>
                              </div>
                            </div>
                          </div>
                  ...
                  {canEdit && dec.status === "PROPOSED" && (
                    <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                      <button
                        onClick={() => updateDecision(dec.id, { status: "APPROVED" })}
                        style={{
                          background: "#065F46",
                      ...
                      <button
                        onClick={() => updateDecision(dec.id, { status: "REJECTED" })}
                        style={{
                          background: "#7F1D1D",
                  ...
                  {canEdit && dec.status === "APPROVED" && (
                    <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                      <button
                        onClick={() => updateDecision(dec.id, { status: "SUPERSEDED" })}
                        style={{
                          background: "var(--hover-bg)",
                          border: "1px solid var(--border)",
                          borderRadius: 4,
                      ...
                      {editingId === dec.id ? (
                        <EditDecisionForm
                          ...
                          onSave={async (updates) => {
                            await updateDecision(dec.id, updates);
                            setEditingId(null);
                          }}
                        />
                      ) : (
                  ...
                  {/* Links */}
                  {(dec.linkedIssueIds.length > 0 ||
                    dec.linkedFlightTestIds.length > 0 ||
                    dec.linkedGateConditions.length > 0) && (
                    <div>
                      <div
                        style={{
                          fontSize: 12,
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {dec.linkedIssueIds.map((id) => (
                          <button
                            key={id}
                            onClick={() => onViewIssue && onViewIssue(id)}
                            style={{
                              background: "#EF444410",
      ...
      {decisions.length === 0 && allDecisions.length === 0 && (
        <EmptyState
          icon={EMPTY_MESSAGES[lang].decisions.icon}
          title={EMPTY_MESSAGES[lang].decisions.title}
          description={EMPTY_MESSAGES[lang].decisions.desc}
        />
      )}
      {decisions.length === 0 && allDecisions.length > 0 && (
        <div
        ...
        >
          ...
          <div style={{ fontSize: 14, color: "var(--text-faint)" }}>
            {lang === "vi" ? "Không có quyết định khớp bộ lọc" : "No decisions match filter"}
          </div>
        </div>
      )}
    </div>
...
function CreateDecisionForm({ lang, project, onClose, onCreate }) {
  ...
  const [form, setForm] = useState({
    title: "",
    titleVi: "",
    phase: project?.currentPhase || "DVT",
    decisionMaker: "",
    rationale: "",
    rationaleVi: "",
    costImpact: "",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          {form.options.map((opt, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 6 }}>
              ...
              <input
                ...
                placeholder={vi ? "Ưu điểm" : "Pros"}
              />
              <input
                style={inputStyle}
                value={opt.cons}
                onChange={(e) => updateOption(i, "cons", e.target.value)}
                placeholder={vi ? "Nhược điểm" : "Cons"}
              />
            </div>
          ))}
        </div>
        ...
        <div style={{ gridColumn: "1 / -1", display: "flex", gap: 6, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            disabled={!form.title || !form.decisionMaker}
            style={{
              background: "#1D4ED8",
              ...
              fontSize: 10,
              fontWeight: 600,
              cursor: !form.title || !form.decisionMaker ? "not-allowed" : "pointer",
              opacity: !form.title || !form.decisionMaker ? 0.4 : 1,
              fontFamily: sans,
              display: "inline-flex",
              alignItems: "center",
              gap: 4,
        ...
        <div>
          ...
          >
            ...
            {decision.options.map((opt, i) => (
              <option key={i} value={opt.label.split(":")[0]}>
                {opt.label}
              </option>
            ))}
          </select>
