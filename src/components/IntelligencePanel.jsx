  ...
  return (
    <div
      style={{
        ...
        border: `1px solid ${hasConvergence ? "#EF444440" : "var(--border)"}`,
        borderRadius: 8,
        padding: 14,
        position: "relative",
        overflow: "hidden",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 6 }}>
        <div style={{ fontSize: 28, fontWeight: 800, fontFamily: mono, color, lineHeight: 1 }}>{Math.round(value)}</div>
          ...
          {Object.entries(score.components).map(([id, comp]) => (
            ...
            >
              ...
              <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>
                ...
                <span style={{ color: "var(--text-faint)", fontSize: 9 }}>({comp.signalCount} sig)</span>
              </span>
            </div>
          ))}
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 10, color: "var(--text-faint)", fontFamily: mono }}>
              {alert.signalTypes.length} {t.intel?.signalTypes || "types"} / {alert.totalEvents}{" "}
              {t.intel?.events || "events"}
            </span>
          </div>
          <div style={{ fontSize: 12, color: "var(--text-primary)", fontWeight: 600, fontFamily: sans }}>
            {Object.entries(alert.dimensionValues)
              .map(([k, v]) => `${k}: ${v}`)
              .join(" / ")}
          </div>
        </div>
        <div style={{ fontSize: 18, fontWeight: 800, fontFamily: mono, color }}>{alert.score}</div>
      </div>
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 16, fontWeight: 800, fontFamily: mono, color }}>{anomaly.multiplier}x</div>
          <div style={{ fontSize: 9, color: "var(--text-faint)", fontFamily: mono }}>z={anomaly.zScore}</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 12, marginTop: 6, fontSize: 10, fontFamily: mono, color: "var(--text-dim)" }}>
        <span>
          {t.intel?.observed || "Observed"}: {anomaly.observedValue}
        </span>
        <span>
          {t.intel?.expected || "Expected"}: {anomaly.expectedValue}
        </span>
      </div>
    </div>
  );
...
export default function IntelligencePanel({ intel, projects, lang, t, onNavigateIssue, issues }) {
  ...
  const subTabs = [
    ...
    },
    {
      ...
      Icon: Zap,
      badge: convergences.length,
    },
    {
      id: "anomaly",
      label: t.intel?.anomaly || (lang === "vi" ? "B\u1EA5t th\u01B0\u1EDDng" : "Anomaly"),
      Icon: BarChart3,
      badge: anomalies.length,
    },
    {
      id: "freshness",
      label: t.intel?.freshness || (lang === "vi" ? "\u0110\u1ED9 t\u01B0\u01A1i" : "Freshness"),
      Icon: RefreshCw,
    },
      ...
      <Section
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 10, width: "100%" }}>
            ...
            <span>
              {t.intel?.title || (lang === "vi" ? "Tr\u00ED tu\u1EC7 D\u1EF1 \u00E1n" : "Project Intelligence")}
            </span>
          ...
          {subTabs.map((st) => (
            <button
              ...
              style={{
                ...
                padding: "6px 12px",
                color: subTab === st.id ? "var(--text-primary)" : "var(--text-dim)",
                fontSize: 12,
                fontWeight: 600,
                cursor: "pointer",
                display: "flex",
        ...
        {subTab === "overview" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              ...
              >
                <Shield size={13} />
                {t.intel?.projectHealth ||
                  (lang === "vi" ? "S\u1EE9c kh\u1ECFe D\u1EF1 \u00E1n (PHI)" : "Project Health Index (PHI)")}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 10 }}>
                {projects.map((p) => {
                  ...
                  return <PHICard key={p.id} score={score} project={p} t={t} convergences={projConvergences} />;
                })}
              </div>
            </div>
            ...
            {convergences.length > 0 && (
              <div>
                ...
                >
                  ...
                  {t.intel?.recentConvergences ||
                    (lang === "vi" ? "H\u1ED9i t\u1EE5 g\u1EA7n \u0111\u00E2y" : "Recent Convergences")}
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {convergences.length === 0 ? (
              <EmptyState
                icon={Zap}
                text={
                  t.intel?.noConvergences ||
                  (lang === "vi"
                    ? "Kh\u00F4ng ph\u00E1t hi\u1EC7n h\u1ED9i t\u1EE5 n\u00E0o"
                    : "No convergences detected")
                }
                sub={
                  t.intel?.noConvergencesSub ||
                  (lang === "vi"
                    ? "H\u1EC7 th\u1ED1ng s\u1EBD ph\u00E1t hi\u1EC7n khi nhi\u1EC1u lo\u1EA1i t\u00EDn hi\u1EC7u xu\u1EA5t hi\u1EC7n c\u00F9ng v\u1ECB tr\u00ED"
                    : "System will detect when multiple signal types converge in the same space")
                }
              />
            ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {anomalies.length === 0 ? (
              <EmptyState
                ...
                text={
                  t.intel?.noAnomalies ||
                  (lang === "vi" ? "Kh\u00F4ng ph\u00E1t hi\u1EC7n b\u1EA5t th\u01B0\u1EDDng" : "No anomalies detected")
                }
                sub={
                  t.intel?.noAnomaliesSub ||
                  (lang === "vi"
                    ? "H\u1EC7 th\u1ED1ng c\u1EA7n t\u00EDch l\u0169y d\u1EEF li\u1EC7u baseline \u0111\u1EC3 so s\u00E1nh"
                    : "System needs to accumulate baseline data for comparison")
                }
              />
            ) : (
        ...
        {subTab === "freshness" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 8 }}>
              <MetricBox
                label={t.intel?.freshSources || "Fresh"}
                value={freshness.fresh}
                icon={CheckCircle2}
                color="#10B981"
              />
              <MetricBox
                label={t.intel?.staleSources || "Stale"}
                value={freshness.stale}
                icon={Clock}
                color="#F59E0B"
              />
              <MetricBox
                label={t.intel?.errorSources || "Error"}
                value={freshness.error}
                icon={XCircle}
                color="#EF4444"
              />
              <MetricBox
                label={t.intel?.coverage || "Coverage"}
                value={`${freshness.coveragePercent}%`}
                icon={Radio}
                color={healthColor}
              />
            ...
            {freshness.degradedDecisions.length > 0 && (
              <div>
                ...
                >
                  ...
                  {t.intel?.degradedDecisions ||
                    (lang === "vi" ? "Quy\u1EBFt \u0111\u1ECBnh b\u1ECB suy gi\u1EA3m" : "Degraded Decisions")}
                </div>
