...
function calculateProjectRisk(project, projectIssues, gateConfig, flightTests, lang) {
  ...
  if (criticalCount > 0)
    factors.push({
      ...
      icon: AlertTriangle,
      detail: isVi
        ? `${criticalCount} vấn đề nghiêm trọng chưa giải quyết`
        : `${criticalCount} unresolved critical issue(s)`,
  ...
  if (blockedCount > 0)
    factors.push({
      ...
      rec: isVi
        ? "Xác định và gỡ bỏ trở ngại. Escalate nếu phụ thuộc bên ngoài"
        : "Identify and remove blockers. Escalate if external dependency",
    });
  ...
  if (gateProgress < 0.8)
    factors.push({
      ...
      icon: DoorOpen,
      detail: isVi
        ? `${Math.round(gateProgress * 100)}% điều kiện bắt buộc đã đạt`
        : `${Math.round(gateProgress * 100)}% required conditions met`,
      rec:
        gateProgress < 0.5
          ? isVi
            ? "Khoảng cách lớn. Rà soát kế hoạch test và phân bổ nguồn lực"
            : "Significant gap. Review test plan and allocate resources"
          : isVi
            ? "Đúng hướng nhưng cần chú ý các điều kiện còn lại"
            : "On track but needs attention on remaining conditions",
    });
  ...
  if (failRate > 0.15)
    factors.push({
      ...
      icon: Plane,
      detail: isVi
        ? `${failedTests.length}/${projTests.length} bay thử thất bại/một phần (${Math.round(failRate * 100)}%)`
        : `${failedTests.length}/${projTests.length} tests failed/partial (${Math.round(failRate * 100)}%)`,
      rec: isVi
        ? "Rà soát patterns thất bại. Cần phân tích nguyên nhân gốc"
        : "Review failure patterns. Root cause analysis needed",
    });
  ...
  if (cascadeIssues.length > 0)
    factors.push({
      ...
      icon: Zap,
      detail: isVi
        ? `${cascadeIssues.length} vấn đề gây delay dây chuyền`
        : `${cascadeIssues.length} issue(s) causing cascade delays`,
      rec: isVi
        ? "Tập trung vào vấn đề gây cascade để tránh ảnh hưởng lịch trình"
        : "Focus on cascade-causing issues to prevent downstream impact",
    });
  ...
  if (riskScore <= 25) {
    riskLevel = isVi ? "THẤP" : "LOW";
    riskColor = "#22C55E";
  } else if (riskScore <= 50) {
    riskLevel = isVi ? "TRUNG BÌNH" : "MODERATE";
    riskColor = "#F59E0B";
  } else if (riskScore <= 75) {
    riskLevel = isVi ? "CAO" : "HIGH";
    riskColor = "#F97316";
  } else {
    riskLevel = isVi ? "NGHIÊM TRỌNG" : "CRITICAL";
    riskColor = "#EF4444";
  }
    ...
    <div style={{ position: "relative", width: 160, height: 160 }}>
      <SafeResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
        <PieChart>
          ...
          >
            {gaugeData.map((d, i) => (
              <Cell key={i} fill={i === 0 ? d.color : "var(--hover-bg)"} />
            ))}
          </Pie>
        </PieChart>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div className="risk-panel-flex" style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, minWidth: 160 }}>
            <RiskGauge
              score={featured.risk.riskScore}
              color={featured.risk.riskColor}
              level={featured.risk.riskLevel}
            />
            <div
              style={{
                fontSize: 13,
                  ...
                  >
                    <div
                      style={{
                        height: "100%",
                        width: `${pct}%`,
                        background: pct > 70 ? "#EF4444" : pct > 40 ? "#F59E0B" : "#3B82F6",
                        borderRadius: 3,
            ...
            {riskResults.map((p) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: `1px solid ${p.id === featured.id ? p.risk.riskColor + "40" : "var(--border)"}`,
                }}
              >
                <span style={{ fontFamily: mono, fontSize: 11, color: "#3B82F6", fontWeight: 700, width: 55 }}>
                  {p.id}
                </span>
                <span
                  style={{
                ...
                <div
                  style={{ width: 60, height: 6, background: "var(--hover-bg)", borderRadius: 3, overflow: "hidden" }}
                >
                  <div
                    style={{
                      height: "100%",
                      width: `${p.risk.riskScore}%`,
                      background: p.risk.riskColor,
                      borderRadius: 3,
                    }}
                  />
                </div>
                ...
                <span
                  style={{ fontSize: 9, fontWeight: 700, color: p.risk.riskColor, letterSpacing: "0.05em", width: 55 }}
                >
                  {p.risk.riskLevel}
                </span>
              </div>
