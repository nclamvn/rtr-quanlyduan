...
import { SUPPLIER_QUAL_COLORS, DELIVERY_STATUS_COLORS } from "../data/v2Data";
...
function StarRating({ rating, max = 5 }) {
  return (
    <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
      ...
      <span style={{ fontSize: 12, fontFamily: mono, color: "var(--text-muted)", marginLeft: 4 }}>
        {rating.toFixed(1)}
      </span>
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
        ...
        <span style={{ fontSize: 12, fontFamily: mono, color: isWarn ? "#EF4444" : color, fontWeight: 700 }}>
          {value}
          {unit}
        </span>
      </div>
...
export default function SupplierModule({ lang, t: _t, project, perm }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <Badge
            label={s.qualificationStatus}
            color={qualColor}
            icon={s.qualificationStatus === "QUALIFIED" ? ShieldCheck : ShieldAlert}
          />
        </div>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          >
            ...
            <div style={{ marginBottom: 10 }}>
              <span
                style={{ fontSize: 11, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em" }}
              >
            ...
            <ScoreBar
              label={lang === "vi" ? "Giao hàng đúng hạn" : "On-Time Delivery"}
              value={s.deliveryOnTimeRate}
              warn={85}
            />
            <ScoreBar
              label={lang === "vi" ? "Tỷ lệ lỗi" : "Defect Rate"}
              value={s.defectRate}
              unit="%"
              max={5}
              color="#EF4444"
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
              <span style={{ fontSize: 12, color: "var(--text-dim)" }}>
                ...
                <span style={{ color: "var(--text-primary)", fontFamily: mono }}>{s.totalOrders}</span>
              </span>
              <span style={{ fontSize: 12, color: "var(--text-dim)" }}>
                {lang === "vi" ? "Trễ" : "Late"}:{" "}
                <span style={{ color: s.lateDeliveries > 0 ? "#F97316" : "#10B981", fontFamily: mono }}>
                  {s.lateDeliveries}
                </span>
              </span>
            </div>
            <div style={{ marginTop: 8, fontSize: 12, color: "var(--text-dim)" }}>
              {lang === "vi" ? "Audit tiếp theo" : "Next Audit"}:{" "}
              <span style={{ fontFamily: mono, color: "var(--text-muted)" }}>{s.nextAuditDate}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {suppliedParts.map((p) => (
              ...
              >
                <span style={{ fontSize: 12, fontFamily: mono, color: "var(--text-dim)", width: 100 }}>
                  {p.partNumber}
                </span>
                <span style={{ fontSize: 13, color: "var(--text-secondary)", flex: 1 }}>
                  {lang === "vi" && p.descriptionVi ? p.descriptionVi : p.description}
                </span>
                <span style={{ fontSize: 12, fontFamily: mono, color: "#10B981" }}>
                  ${p.unitCost}/{p.unit}
                </span>
                ...
                <div key={d.id} style={{ position: "relative" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 12, fontFamily: mono, color: "var(--text-dim)" }}>
                      {d.actualDate || d.promisedDate}
                    </span>
                    <span style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                      {part?.partNumber || d.bomPartId}
                    </span>
                    ...
                    {d.delayDays > 0 && (
                      <span style={{ fontSize: 11, color: "#F97316", fontWeight: 700 }}>+{d.delayDays}d</span>
                    )}
                  </div>
                </div>
              );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, color: "var(--text-dim)" }}>
          — {suppliers.length} {lang === "vi" ? "nhà cung cấp" : "suppliers"}
        </span>
        {readOnly && (
          <span
            style={{
      ...
      {suppliers.length === 0 && (
        <EmptyState
          icon={EMPTY_MESSAGES[lang].suppliers.icon}
          title={EMPTY_MESSAGES[lang].suppliers.title}
          description={EMPTY_MESSAGES[lang].suppliers.desc}
        />
      )}
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, color: "var(--text-faint)", textTransform: "uppercase" }}>
                    {lang === "vi" ? "Đúng hạn" : "On-Time"}
                  </div>
                  <span
                    style={{
                      fontSize: 14,
                      fontFamily: mono,
                      color: s.deliveryOnTimeRate >= 90 ? "#10B981" : "#F59E0B",
                      fontWeight: 700,
                    }}
                  >
                    {s.deliveryOnTimeRate}%
                  </span>
                </div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--text-dim)" }}>
                <span>
                  <Package size={10} style={{ display: "inline", verticalAlign: "middle" }} /> {partsCount}{" "}
                  {lang === "vi" ? "linh kiện" : "parts"}
                </span>
                <span>{s.country}</span>
              </div>
            </div>
