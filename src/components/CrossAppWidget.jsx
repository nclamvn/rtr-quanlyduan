...
export default function CrossAppWidget({ data, summary: _summary, loading, lang }) {
  const vi = lang === "vi";
    ...
    return (
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="skeleton" style={{ height: 56, flex: 1, borderRadius: 6 }} />
          ))}
        </div>
      </div>
    );
  ...
  const overdueWOs = workOrders.filter(
    (d) => d.due_date && new Date(d.due_date) < new Date() && !["completed", "cancelled"].includes(d.status),
  );
      ...
      >
        ...
        {lastSynced && (
          <span style={{ fontSize: 9, color: "var(--text-faint)", fontFamily: mono }}>
            {vi ? "Đồng bộ" : "Synced"}{" "}
            {new Date(lastSynced).toLocaleTimeString(vi ? "vi-VN" : "en-US", { hour: "2-digit", minute: "2-digit" })}
          </span>
        )}
      </div>
        ...
        <div style={{ padding: 16 }}>
          ...
          {prodSummary && (
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              <MiniCard
                icon={Factory}
                label={vi ? "Đang SX" : "In Prod"}
                value={prodSummary.inProduction}
                color="#F59E0B"
              />
              <MiniCard
                icon={CheckCircle2}
                label={vi ? "Đã xong" : "Done"}
                value={prodSummary.completedToday}
                color="#10B981"
              />
              <MiniCard icon={Clock} label={vi ? "Chờ" : "Queued"} value={prodSummary.confirmed} color="#3B82F6" />
              <MiniCard
                icon={Ban}
                label={vi ? "Trễ" : "Overdue"}
                value={prodSummary.overdue}
                color={prodSummary.overdue > 0 ? "#EF4444" : "#10B981"}
              />
            </div>
          )}
          ...
          <div style={{ maxHeight: 200, overflowY: "auto" }}>
            {activeTab === "production" &&
              workOrders.slice(0, 15).map((wo) => (
                ...
                >
                  <span
                    style={{ fontFamily: mono, color: "var(--text-faint)", fontSize: 10, width: 70, flexShrink: 0 }}
                  >
                    {wo.data?.woNumber}
                  </span>
                  <span
                  ...
                  <span style={{ fontFamily: mono, color: "var(--text-muted)", fontSize: 11 }}>
                    ×{wo.data?.quantity}
                  </span>
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                      color: STATUS_COLORS[wo.status] || "var(--text-dim)",
                      textTransform: "uppercase",
                      fontFamily: mono,
                    }}
                  >
            ...
            {activeTab === "inventory" &&
              inventoryAlerts.slice(0, 15).map((inv) => (
                <div
                  key={inv.entity_id}
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  <span
                    style={{
                      fontFamily: mono,
                      fontSize: 11,
                      color: inv.data?.availableQty <= 0 ? "#EF4444" : "#F59E0B",
                      fontWeight: 700,
                    }}
                  >
                    {inv.data?.availableQty}/{inv.data?.reorderPoint}
                  </span>
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                      color: STATUS_COLORS[inv.status] || "#F59E0B",
                      textTransform: "uppercase",
                      fontFamily: mono,
                    }}
                  >
            ...
            {activeTab === "sales" &&
              salesOrders.slice(0, 15).map((so) => (
                ...
                >
                  ...
                  >
                    {so.data?.customerName}
                  </span>
                  {so.data?.totalAmount && (
                    <span style={{ fontFamily: mono, fontSize: 11, color: "#10B981" }}>
                      ${so.data.totalAmount.toLocaleString()}
                    </span>
                  )}
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                      color: STATUS_COLORS[so.status] || "var(--text-dim)",
                      textTransform: "uppercase",
                      fontFamily: mono,
                    }}
                  >
            ...
            {((activeTab === "production" && workOrders.length === 0) ||
              ...
              <div
                style={{ padding: 20, textAlign: "center", color: "var(--text-faint)", fontSize: 12, fontFamily: sans }}
              >
