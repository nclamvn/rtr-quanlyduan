...
const IMPORT_TYPES = [
  {
    ...
    label: "Issues",
    labelVi: "Vấn Đề",
    desc: "Import issue tracking data",
    descVi: "Import dữ liệu theo dõi vấn đề",
  },
  {
    id: "bom",
    icon: Package,
    label: "BOM",
    labelVi: "BOM",
    desc: "Import Bill of Materials",
    descVi: "Import danh sách linh kiện",
  },
  {
    id: "flightTests",
    icon: Plane,
    label: "Flight Tests",
    labelVi: "Bay Thử",
    desc: "Import test log records",
    descVi: "Import nhật ký bay thử",
  },
  {
    id: "milestones",
    icon: Milestone,
    label: "Milestones",
    labelVi: "Mốc",
    desc: "Import phase timeline data",
    descVi: "Import mốc thời gian",
  },
  {
    id: "orders",
    icon: ShoppingCart,
    label: "Orders",
    labelVi: "Đơn Hàng",
    desc: "Import customer orders",
    descVi: "Import đơn hàng khách hàng",
  },
  {
    id: "production",
    icon: Factory,
  ...
  },
  {
    ...
    label: "Inventory",
    labelVi: "Tồn Kho",
    desc: "Import inventory items",
    descVi: "Import dữ liệu tồn kho",
  },
  {
    id: "costs",
    icon: DollarSign,
    label: "Costs",
    labelVi: "Chi Phí",
    desc: "Import cost entries",
    descVi: "Import dữ liệu chi phí",
  },
];
...
export default function ImportWizard({ lang, project: _project, issues = [], onImport, onClose, initialType }) {
  ...
  const [_valueMapping, _setValueMapping] = useState({});
  ...
  const btnStyle = (primary) => ({
    ...
    border: `1px solid ${primary ? "#3B82F6" : "var(--text-disabled)"}`,
    borderRadius: 4,
    padding: "6px 16px",
      ...
      <div style={modalStyle}>
            ...
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <div
                style={{
                  ...
                  color: step >= s.id ? "#fff" : "var(--text-faint)",
                  fontSize: 12,
                  fontWeight: 700,
                }}
              >
              ...
              <span
                style={{
                  fontSize: 12,
                  color: step >= s.id ? "var(--text-primary)" : "var(--text-faint)",
        ...
        <div style={bodyStyle}>
            ...
            <div>
              ...
              <div
                ...
                style={{
                  ...
                  textAlign: "center",
                  cursor: "pointer",
                  background: isDragging ? "#3B82F608" : "var(--bg-card)",
                  transition: "all 0.2s",
                }}
              >
                <Upload
                  size={32}
                  color={isDragging ? "#3B82F6" : "var(--text-faint)"}
                  style={{ margin: "0 auto 8px" }}
                />
                <div style={{ fontSize: 14, color: "var(--text-muted)" }}>
                  {t ? "Kéo thả file vào đây hoặc click để chọn" : "Drag & drop your file here or click to browse"}
                </div>
                <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 4 }}>.xlsx .xls .csv</div>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept=".xlsx,.xls,.csv"
              ...
              {file && workbookData && (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 12, fontFamily: mono, color: "var(--text-dim)" }}>
                  ...
                  <div style={{ display: "flex", gap: 12 }}>
                    ...
                    <div>
                      ...
                      >
                        {Array.from(
                          { length: Math.min(10, workbookData.sheets[selectedSheet]?.length || 1) },
                          (_, i) => (
            ...
            <div>
              ...
              >
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns: "1fr 30px 1fr 100px",
                    gap: 8,
                    padding: "8px 12px",
                    borderBottom: "1px solid var(--border)",
                    fontSize: 11,
                      ...
                      >
                        ...
                        {contextFields.map((f) => (
                          <option key={f} value={f}>
                            {f} {IMPORT_TYPE_FIELDS[importType]?.required.includes(f) ? "*" : ""}
                          </option>
                        ))}
                      </select>
                      <span
                        style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: confStyle.color }}
                      >
              ...
              {(() => {
                const config = IMPORT_TYPE_FIELDS[importType];
                const enumFields = Object.entries(config.enums).filter(([field]) =>
                  Object.values(effectiveMapping).includes(field),
                );
          ...
          {step === 4 && (
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", marginBottom: 4 }}>
                {t ? "Xem trước dữ liệu" : "Preview"}: {previewRows.length}{" "}
                {t ? "dòng sẽ được nhập" : "items will be imported"}
              </div>
              <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                <span style={{ fontSize: 13, color: "#10B981" }}>
                  <Check size={10} style={{ display: "inline", verticalAlign: "middle" }} /> {validCount}{" "}
                  {t ? "hợp lệ" : "valid"}
                </span>
                <span style={{ fontSize: 13, color: "#F59E0B" }}>
                  <TriangleAlert size={10} style={{ display: "inline", verticalAlign: "middle" }} /> {warnCount}{" "}
                  {t ? "cảnh báo" : "warnings"}
                </span>
                <span style={{ fontSize: 13, color: "#EF4444" }}>
                  <X size={10} style={{ display: "inline", verticalAlign: "middle" }} /> {errorCount}{" "}
                  {t ? "lỗi" : "errors"}
                </span>
              </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "var(--bg-modal)", borderBottom: "1px solid var(--border)" }}>
                      ...
                      {Object.entries(effectiveMapping)
                        .filter(([, v]) => v && v !== "_skip")
                        .slice(0, 6)
                        .map(([, field]) => (
                          <th
                  ...
                  <tbody>
                    {previewRows.slice(0, 50).map((row, i) => {
                      ...
                      const rowColor =
                        v.status === "error" ? "#EF444410" : v.status === "warning" ? "#F59E0B08" : "transparent";
                      return (
              ...
              <div style={{ marginTop: 12, display: "flex", gap: 16, alignItems: "center" }}>
                ...
                >
                  <input
                    type="checkbox"
                    checked={importOptions.asDraft}
                    onChange={(e) => setImportOptions((o) => ({ ...o, asDraft: e.target.checked }))}
                    style={{ accentColor: "#3B82F6" }}
                  />
                  {t
                    ? "Nhập dạng DRAFT (PM duyệt trước khi kích hoạt)"
                    : "Import as DRAFT (PM reviews before activating)"}
                </label>
              </div>
            </div>
          )}
          ...
          {step === 5 && importResult && (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 16 }}>
                ...
                >
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#10B981", fontFamily: mono }}>
                ...
                {importResult.warnings > 0 && (
                  ...
                  >
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#F59E0B", fontFamily: mono }}>
                ...
                {importResult.skipped > 0 && (
                  ...
                  >
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#EF4444", fontFamily: mono }}>
              ...
              <div style={{ fontSize: 13, color: "var(--text-dim)", marginTop: 16 }}>
                {t ? "Tổng" : "Total"}: {importResult.imported}/{importResult.total} | {t ? "Trạng thái" : "Status"}:
                DRAFT
              </div>
              <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 8 }}>
                {t
                  ? "Đi đến hàng đợi Duyệt để kích hoạt các mục đã nhập."
                  : "Go to Review Queue to activate imported items."}
              </div>
            </div>
          )}
        </div>
        ...
        <div style={footerStyle}>
          ...
          {step < 4 && (
            <button
              ...
              disabled={!canNext()}
              style={{ ...btnStyle(true), opacity: canNext() ? 1 : 0.4 }}
            >
          ...
          {step === 4 && (
            <button
              ...
              style={{ ...btnStyle(true), opacity: canNext() ? 1 : 0.4 }}
            >
              {t ? `Nhập ${validCount + warnCount} mục` : `Import ${validCount + warnCount} items`}{" "}
