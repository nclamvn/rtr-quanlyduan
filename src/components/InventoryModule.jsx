...
export default function InventoryModule({ inventory, transactions, loading, lang, perm: _perm }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter((i) => i.partNumber.toLowerCase().includes(q) || i.partName.toLowerCase().includes(q));
    }
    return list;
  ...
  if (inventory.length === 0) {
    const em = EMPTY_MESSAGES[lang]?.inventory || EMPTY_MESSAGES.vi.inventory;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        {[
          ...
          {
            label: t ? "Cảnh báo thấp" : "Low Stock",
            value: lowStockCount,
            icon: AlertTriangle,
            color: lowStockCount > 0 ? "#F59E0B" : "#64748B",
          },
          {
            label: t ? "Hết hàng" : "Critical/Out",
            value: criticalCount,
            icon: AlertTriangle,
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          ...
          <input
            ...
            placeholder={t ? "Tìm linh kiện..." : "Search parts..."}
            style={{
              width: "100%",
      ...
      >
              ...
              {filtered.map((item) => (
                <tr
                  ...
                  style={{
                    ...
                    background: selItem?.id === item.id ? "var(--hover-bg)" : "transparent",
                  }}
                >
                  <td style={{ padding: "8px 12px", fontWeight: 700, fontFamily: "monospace", fontSize: 12 }}>
                    {item.partNumber}
                  </td>
                  ...
                  <td style={{ padding: "8px 12px", fontWeight: 700, color: STOCK_COLORS[item.stockStatus] }}>
        ...
        {filtered.length === 0 && (
          <div style={{ padding: 30, textAlign: "center", color: "var(--text-dim)" }}>
            {t ? "Không tìm thấy" : "No items match"}
          </div>
        )}
      </div>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 800 }}>
              {selItem.partNumber} — {selItem.partName}
            </div>
            <button
          ...
          >
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Kho:" : "Warehouse:"}</span> {selItem.warehouse}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Vị trí:" : "Location:"}</span> {selItem.location || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Loại:" : "Category:"}</span> {selItem.category || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "NCC:" : "Supplier:"}</span> {selItem.supplierName || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Đơn giá:" : "Unit Cost:"}</span> $
              {selItem.unitCost.toFixed(2)}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Đang đặt:" : "On Order:"}</span>{" "}
              {selItem.quantityOnOrder}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>Lead Time:</span> {selItem.leadTimeDays} {t ? "ngày" : "days"}
            </div>
          </div>
          {selItem.notes && (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {itemTransactions.slice(0, 10).map((tx) => (
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 700, color: tx.quantity > 0 ? "#10B981" : "#EF4444" }}>
                      {tx.quantity > 0 ? "+" : ""}
                      {tx.quantity}
                    </span>
          ...
          <div style={{ padding: "0 14px 14px", display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6 }}>
                {t ? "Top cảnh báo tồn kho" : "Low Stock Alerts"}
              </div>
              {inventory
                ...
                .map((i) => (
                  ...
                  >
                    ...
                    <span style={{ marginLeft: "auto", fontWeight: 700, color: STOCK_COLORS[i.stockStatus] }}>
                      {i.quantityAvailable}
                    </span>
                  </div>
                ))}
