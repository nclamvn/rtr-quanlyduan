...
import {
  Brain, Upload, FileSpreadsheet, X, Check, ChevronRight,
  AlertTriangle, Package, Plane, Milestone as MilestoneIcon,
  ShoppingCart, Factory, Warehouse, DollarSign, Zap,
  CheckCircle2, CircleAlert, RefreshCw, ArrowRight, Layers,
} from "lucide-react";
...
function ConfidenceBadge({ confidence }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "1px 6px", borderRadius: 3, background: color + "15", color, fontSize: 9, fontWeight: 700, fontFamily: mono, border: `1px solid ${color}25` }}>
      {confidence}% {label}
    </span>
  );
}
...
export default function AIImportWizard({ lang, project, onImport, onClose }) {
  ...
  const handleImportAll = useCallback(async () => {
    ...
    for (const cls of sheetsToImport) {
      ...
      if (!transform || transform.mappedRows.length === 0) {
        results.push({ sheetName: cls.sheetName, type: cls.type, status: "skipped", count: 0, reason: "No valid rows" });
        continue;
      }
      ...
      try {
        ...
        results.push({ sheetName: cls.sheetName, type: cls.type, status: "success", count: transform.mappedRows.length });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "var(--overlay)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "var(--bg-modal)", border: "1px solid var(--border)", borderRadius: 12, width: "90%", maxWidth: 800, maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          {step === 1 && (
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "var(--text-dim)", marginBottom: 12 }}>
                {vi ? "hoặc click để chọn file • .xlsx, .xls, .csv • Tối đa 5MB" : "or click to browse • .xlsx, .xls, .csv • Max 5MB"}
              </div>
              <div style={{ fontSize: 12, color: "var(--text-faint)", background: "var(--bg-card)", display: "inline-block", padding: "6px 14px", borderRadius: 6, border: "1px solid var(--border)" }}>
                <Brain size={12} style={{ display: "inline", verticalAlign: "middle", marginRight: 4 }} />
                {vi ? "AI sẽ tự động nhận diện Issues, BOM, Orders, Production, Inventory..." : "AI auto-detects Issues, BOM, Orders, Production, Inventory..."}
              </div>
          ...
          {fileError && (
            <div style={{ marginTop: 12, padding: "8px 14px", background: "#EF444415", border: "1px solid #EF444430", borderRadius: 6, color: "#FCA5A5", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {step === 2 && (
            <div style={{ textAlign: "center", padding: 60 }}>
              ...
              <div style={{ fontSize: 13, color: "var(--text-dim)", marginTop: 4 }}>
                {vi ? "Nhận diện loại dữ liệu, ánh xạ cột, kiểm tra dữ liệu" : "Detecting data types, mapping columns, validating data"}
              </div>
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {/* Summary bar */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Zap size={14} color="#F59E0B" />
                  <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>
                    {vi ? `AI phát hiện ${classifications.length} sheets` : `AI detected ${classifications.length} sheets`}
                  </span>
                </div>
              ...
              {classifications.map((cls) => {
                ...
                return (
                  <div key={cls.sheetName} style={{
                    background: "var(--bg-card)", border: `1px solid ${isSelected ? meta.color + "40" : "var(--border)"}`,
                    borderRadius: 8, overflow: "hidden", opacity: isSelected ? 1 : 0.6,
                    transition: "all 0.2s",
                  }}>
                    {/* Card header */}
                    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: "1px solid var(--border)" }}>
                      {/* Checkbox */}
                      <button onClick={() => setSelectedSheets(prev => ({ ...prev, [cls.sheetName]: !prev[cls.sheetName] }))}
                        style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${isSelected ? meta.color : "var(--text-faint)"}`, background: isSelected ? meta.color : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                        {isSelected && <Check size={10} color="#fff" strokeWidth={3} />}
                      </button>

                      {/* Sheet icon + name */}
                      <div style={{ width: 28, height: 28, borderRadius: 6, background: meta.color + "15", border: `1px solid ${meta.color}25`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        ...
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)", fontFamily: mono }}>
                      ...
                      <div style={{ padding: "8px 14px", display: "flex", gap: 4, flexWrap: "wrap" }}>
                        {Object.entries(cls.matchedFields).map(([header, match]) => (
                          <span key={header} style={{
                            padding: "1px 6px", borderRadius: 3, fontSize: 10, fontFamily: mono,
                    ...
                    {(isSelected || cls.confidence < 50) && transform?.rawPreview?.length > 0 && (
                      <div style={{ padding: "8px 14px", borderTop: "1px solid var(--border)" }}>
                        <div style={{ fontSize: 10, color: "var(--text-faint)", marginBottom: 4, fontWeight: 700, textTransform: "uppercase" }}>
                            ...
                            <thead>
                              <tr>
                                {cls.headers.slice(0, 8).map((h, i) => (
                                  <th key={i} style={{ padding: "2px 6px", textAlign: "left", color: "var(--text-dim)", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis" }}>{h}</th>
                                ))}
                                {cls.headers.length > 8 && <th style={{ padding: "2px 6px", color: "var(--text-faint)" }}>+{cls.headers.length - 8}</th>}
                              </tr>
                            </thead>
                            <tbody>
                              {transform.rawPreview.slice(0, 3).map((row, ri) => (
                                <tr key={ri}>
                                  {cls.headers.slice(0, 8).map((h, ci) => (
                                    <td key={ci} style={{ padding: "2px 6px", color: "var(--text-secondary)", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis" }}>
          ...
          {step === 4 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {classifications.filter(c => selectedSheets[c.sheetName]).map((cls) => {
                const meta = TYPE_META[cls.type] || TYPE_META.issues;
                const Icon = meta.icon;
                const progress = importProgress[cls.sheetName];
                return (
                  <div key={cls.sheetName} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 6 }}>
                    <Icon size={14} color={meta.color} />
                    <span style={{ flex: 1, fontSize: 13, color: "var(--text-primary)" }}>{cls.sheetName}</span>
                    {progress === "importing" && <RefreshCw size={12} color="#3B82F6" style={{ animation: "spin 1s linear infinite" }} />}
                    {progress === "done" && <CheckCircle2 size={14} color="#10B981" />}
                    {progress === "error" && <CircleAlert size={14} color="#EF4444" />}
                    {!progress && <span style={{ fontSize: 11, color: "var(--text-faint)" }}>{vi ? "Chờ..." : "Waiting..."}</span>}
                  </div>
                );
              })}
            </div>
          ...
          {step === 5 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {importResults.map((r) => {
                ...
                const Icon = meta.icon;
                return (
                  <div key={r.sheetName} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 6 }}>
                    <Icon size={14} color={meta.color} />
                    <span style={{ flex: 1, fontSize: 13, color: "var(--text-primary)" }}>{r.sheetName}</span>
                    {r.status === "success" && (
                      <span style={{ fontSize: 12, color: "#10B981", fontWeight: 700 }}>
                        <Check size={10} style={{ display: "inline", verticalAlign: "middle" }} /> {r.count} {vi ? "bản ghi" : "records"}
                      </span>
                    )}
                    {r.status === "error" && (
                      <span style={{ fontSize: 12, color: "#EF4444" }}>{r.reason}</span>
                    )}
                    {r.status === "skipped" && (
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderTop: "1px solid var(--border)" }}>
          <div style={{ fontSize: 12, color: "var(--text-faint)" }}>
            {step === 3 && `${totalSelected} ${vi ? "sheets đã chọn" : "sheets selected"} • ${totalRows} ${vi ? "hàng" : "rows"}`}
            {step === 5 && `${importResults.filter(r => r.status === "success").reduce((s, r) => s + r.count, 0)} ${vi ? "bản ghi đã import" : "records imported"}`}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {step < 4 && (
              <button onClick={onClose}
                style={{ background: "var(--hover-bg)", border: "1px solid var(--border)", borderRadius: 6, padding: "6px 16px", color: "var(--text-secondary)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: sans }}>
                {vi ? "Hủy" : "Cancel"}
              </button>
            )}
            {step === 3 && totalSelected > 0 && (
              <button onClick={handleImportAll}
                style={{ background: "linear-gradient(135deg, #8B5CF6, #3B82F6)", border: "none", borderRadius: 6, padding: "6px 20px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: sans, display: "flex", alignItems: "center", gap: 6 }}>
                <Brain size={14} />
                {vi ? `Import ${totalSelected} sheets (${totalRows} hàng)` : `Import ${totalSelected} sheets (${totalRows} rows)`}
              </button>
            )}
            {step === 5 && (
              <button onClick={onClose}
                style={{ background: "#10B981", border: "none", borderRadius: 6, padding: "6px 20px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: sans }}>
                {vi ? "Hoàn tất" : "Done"}
              </button>
            )}
          </div>
