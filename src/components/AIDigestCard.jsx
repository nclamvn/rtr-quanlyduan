        ...
        <div style={{ padding: 16 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {digest.criticalItems.map((item, i) => (
                ...
                >
                  <AlertTriangle
                    size={12}
                    color={URGENCY_COLORS[item.urgency] || "#F59E0B"}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: sans, lineHeight: 1.4 }}>
                      {item.detail}
                    </div>
                  </div>
                </div>
          ...
          {digest.patterns?.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <SectionHeader
                icon={TrendingUp}
                label={vi ? "Xu hướng phát hiện" : "Patterns Detected"}
                color="#F59E0B"
              />
              {digest.patterns.map((p, i) => (
                <div key={i} style={{ padding: "6px 0", borderBottom: "1px solid var(--border-a10)" }}>
                  <div style={{ fontSize: 12, color: "var(--text-primary)", fontFamily: sans, fontWeight: 600 }}>
                    {p.observation}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--text-faint)", fontFamily: sans }}>
                    {p.suggestion && (
                      <>
                        <Lightbulb size={9} style={{ display: "inline", verticalAlign: "middle" }} /> {p.suggestion}
                      </>
                    )}
                  </div>
                </div>
          ...
          {digest.teamInsights?.length > 0 && (
            <div>
              ...
              {digest.teamInsights.map((t, i) => (
                <div
                  key={i}
                  style={{ display: "flex", gap: 8, padding: "6px 0", borderBottom: "1px solid var(--border-a10)" }}
                >
                  <div
                    style={{
                      width: 24,
                  ...
                  <div>
                    ...
                    {t.recommendation && (
                      <div style={{ fontSize: 11, color: "#8B5CF6", fontFamily: sans }}>{t.recommendation}</div>
                    )}
                  </div>
                </div>
              ))}
