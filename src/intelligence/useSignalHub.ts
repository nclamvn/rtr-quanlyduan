...
export function useSignalHub(
  ...
  useEffect(() => {
    ...
    const signals = hydrateFromExistingData(
      issues,
      flights,
      deliveries,
      bomItems,
    ...
    if (newItems.length > 0) {
      const signals = newItems
        .filter(
          (w) =>
            w.plannedEnd &&
            new Date(w.plannedEnd) < new Date() &&
  ...
  // Ingest functions
  const ingestIssue = useCallback(
    (issue: any, action: "created" | "updated" | "closed") => {
      if (!hubRef.current) return;
  ...
  const ingestGateToggle = useCallback(
    (projectId: string, phase: string, condId: string, val: boolean) => {
      if (!hubRef.current) return;
  ...
  const ingestFlightResult = useCallback(
    (flight: any) => {
      if (!hubRef.current) return;
  ...
  const ingestDelivery = useCallback(
    (delivery: any) => {
      if (!hubRef.current) return;
  ...
  const getProjectScore = useCallback(
    (projectId: string): IndexScore | undefined => {
      return state.projectScores.find((s) => s.entityId === projectId);
    },
    [state.projectScores],
  );

  const getProjectConvergences = useCallback(
    (projectId: string): ConvergenceAlert[] => {
      return state.convergences.filter((c) => c.dimensionValues.project === projectId);
    },
    [state.convergences],
  );
