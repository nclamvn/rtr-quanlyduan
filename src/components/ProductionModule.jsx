...
export default function ProductionModule({ productionOrders, loading, lang, perm: _perm }) {
  ...
  const onTimeRate =
    completed.length > 0
      ? (
          (completed.filter((w) => !w.plannedEnd || new Date(w.actualEnd || today) <= new Date(w.plannedEnd)).length /
            completed.length) *
          100
        ).toFixed(0)
      : "—";
  const delayed = productionOrders.filter(
    (w) => w.plannedEnd && new Date(w.plannedEnd) < today && !["COMPLETED", "SHIPPED", "CANCELLED"].includes(w.status),
  ).length;
  ...
  if (productionOrders.length === 0) {
    const em = EMPTY_MESSAGES[lang]?.production || EMPTY_MESSAGES.vi.production;
    return <EmptyState icon={em.icon} title={em.title} description={em.description} />;
  }

  const Badge = ({ color, children }) => (
    <span
      style={{ background: color + "18", color, padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 700 }}
    >
      {children}
    </span>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        {[
          ...
          {
            label: t ? "Trễ hạn" : "Delayed",
            value: delayed,
            icon: AlertTriangle,
            color: delayed > 0 ? "#EF4444" : "#64748B",
          },
        ].map((kpi, i) => (
          <div
            key={i}
          ...
          >
            <div style={{ background: kpi.color + "15", borderRadius: 8, padding: 8 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          ...
          <input
            ...
            placeholder={t ? "Tìm WO..." : "Search WO..."}
            style={{
              width: "100%",
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {filtered.map((w) => {
                const isLate =
                  w.plannedEnd &&
                  new Date(w.plannedEnd) < today &&
                  !["COMPLETED", "SHIPPED", "CANCELLED"].includes(w.status);
                return (
                  <tr
                    key={w.id}
                    onClick={() => setSelWO(selWO?.id === w.id ? null : w)}
                    style={{
                      cursor: "pointer",
                      borderBottom: "1px solid var(--border)",
                      background: selWO?.id === w.id ? "var(--hover-bg)" : "transparent",
                    }}
                  >
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      {w.currentStation ? (
                        <Badge color={STATION_COLORS[w.currentStation] || "#64748B"}>{w.currentStation}</Badge>
                      ) : (
                        "—"
                      )}
                    </td>
                    <td style={{ padding: "8px 12px" }}>
                      <Badge color={STATUS_COLORS[w.status] || "#64748B"}>{w.status.replace(/_/g, " ")}</Badge>
                    </td>
                    <td style={{ padding: "8px 12px", fontSize: 12, color: isLate ? "#EF4444" : "var(--text-dim)" }}>
                      {w.plannedStart || "—"} → {w.plannedEnd || "—"}{" "}
                      {isLate && <AlertTriangle size={11} style={{ marginLeft: 2 }} />}
                    </td>
                    <td style={{ padding: "8px 12px" }}>
                      {w.yieldQuantity > 0 || w.defectQuantity > 0 ? (
                        <span>
                          {w.yieldQuantity}
                          <span style={{ color: "var(--text-faint)" }}>/{w.quantity}</span>
                          {w.defectQuantity > 0 && (
                            <span style={{ color: "#EF4444", marginLeft: 4 }}>({w.defectQuantity} fail)</span>
                          )}
                        </span>
                      ) : (
                        "—"
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 800 }}>
              {selWO.woNumber} — {selWO.productName}
            </div>
            <button
              onClick={() => setSelWO(null)}
          ...
          >
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Đơn hàng:" : "Order:"}</span> {selWO.orderNumber || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Khách:" : "Customer:"}</span> {selWO.customerName || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Phụ trách:" : "Assigned:"}</span>{" "}
              {selWO.assignedTo || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Thực tế:" : "Actual:"}</span> {selWO.actualStart || "—"}{" "}
              → {selWO.actualEnd || t ? "đang chạy" : "ongoing"}
            </div>
          </div>
          {selWO.defectNotes && (
            <div
          ...
          {selWO.logs.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                {t ? "Nhật ký sản xuất" : "Production Log"} ({selWO.logs.length})
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {selWO.logs.map((log) => (
                  ...
                  >
                    ...
                    <Badge
                      color={
                        log.action === "FAILED_QC" ? "#EF4444" : log.action === "PASSED_QC" ? "#10B981" : "#3B82F6"
                      }
                    >
                      {log.action}
                    </Badge>
                    ...
                    {log.durationMinutes && (
                      <span style={{ color: "var(--text-faint)" }}>{log.durationMinutes}min</span>
                    )}
