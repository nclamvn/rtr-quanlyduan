...
import { DollarSign, TrendingUp, AlertTriangle, Clock, ChevronDown, ChevronRight } from "lucide-react";
...
export default function FinanceModule({ financeSummary, invoices, costEntries, loading, lang }) {
  ...
  const [_showCharts, _setShowCharts] = useState(true);
  const t = lang === "vi";
  ...
  const costByCat = useMemo(() => {
    ...
    return Object.entries(map).map(([name, value]) => ({
      name,
      value,
      color: COST_CATEGORY_COLORS[name] || "#64748B",
    }));
  }, [costEntries]);
  ...
  const marginByProject = useMemo(
    () =>
      financeSummary
        .filter((f) => f.totalRevenue > 0 || f.totalCosts > 0)
        .map((f) => ({
  ...
  if (financeSummary.length === 0 && invoices.length === 0) {
    const em = EMPTY_MESSAGES[lang]?.finance || EMPTY_MESSAGES.vi.finance;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        {[
          ...
          {
            label: t ? "Biên lợi nhuận" : "Margin",
            value: `${marginPercent}%`,
            icon: TrendingUp,
            color: parseFloat(marginPercent) >= 50 ? "#10B981" : "#F59E0B",
          },
          {
            label: t ? "AR chưa thu" : "Outstanding AR",
            value: fmt(outstandingAR),
            icon: Clock,
            color: outstandingAR > 0 ? "#F59E0B" : "#64748B",
          },
          {
            label: t ? "Hóa đơn quá hạn" : "Overdue Invoices",
            value: overdueInvoices,
            icon: AlertTriangle,
            color: overdueInvoices > 0 ? "#EF4444" : "#64748B",
          },
        ].map((kpi, i) => (
          <div
            key={i}
            ...
            .map((f) => (
              ...
              >
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, fontSize: 12 }}>
                  <div>
                    <span style={{ color: "var(--text-dim)" }}>{t ? "DT:" : "Rev:"}</span>{" "}
                    <span style={{ fontWeight: 700, color: "#10B981" }}>{fmt(f.totalRevenue)}</span>
                  </div>
                  <div>
                    <span style={{ color: "var(--text-dim)" }}>{t ? "CP:" : "Cost:"}</span>{" "}
                    <span style={{ fontWeight: 700, color: "#F59E0B" }}>{fmt(f.totalCosts)}</span>
                  </div>
                  <div>
                    <span style={{ color: "var(--text-dim)" }}>{t ? "BLN:" : "Margin:"}</span>{" "}
                    <span style={{ fontWeight: 700, color: f.marginPercent >= 50 ? "#10B981" : "#F59E0B" }}>
                      {f.marginPercent.toFixed(0)}%
                    </span>
                  </div>
                </div>
                <div style={{ marginTop: 6 }}>
                  <div style={{ height: 6, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
                    <div
                      style={{
                        height: "100%",
                        width: `${Math.min(100, f.marginPercent)}%`,
                        background: f.marginPercent >= 50 ? "#10B981" : "#F59E0B",
                        borderRadius: 3,
                      }}
                    />
                  </div>
                ...
                {f.overdueCount > 0 && (
                  <div style={{ marginTop: 4, fontSize: 11, color: "#EF4444" }}>
                    {f.overdueCount} {t ? "đơn quá hạn" : "overdue"}
                  </div>
                )}
              </div>
            ))}
                  ...
                  <tr key={inv.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    ...
                    <td
                      style={{
                        padding: "8px 12px",
                        fontWeight: 600,
                        color: inv.balance > 0 ? "#F59E0B" : "var(--text-dim)",
                      }}
                    >
                      {fmt(inv.balance)}
                    </td>
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      {inv.daysPastDue > 0 ? (
                        <Badge color={AGING_COLORS[inv.agingBucket]}>{inv.daysPastDue}d</Badge>
                      ) : (
                        "—"
                      )}
                    </td>
              ...
              <tbody>
                {costEntries.map((c) => (
                  <tr key={c.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    <td style={{ padding: "8px 12px" }}>
                      <Badge color={COST_CATEGORY_COLORS[c.category]}>{c.category}</Badge>
                    </td>
      ...
      {subTab === "summary" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 14 }}>
          ...
          {marginByProject.length > 0 && (
            <div
              style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 14 }}
            >
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                {t ? "Doanh thu vs Chi phí" : "Revenue vs Cost"}
              </div>
          ...
          {costByCat.length > 0 && (
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                {t ? "Chi phí theo loại" : "Cost Breakdown"}
              </div>
              <SafeResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie
