...
export default function OrdersModule({ orders, customers, loading, lang, perm: _perm }) {
  ...
  const pipelineValue = orders
    .filter((o) => !["CANCELLED", "CLOSED", "PAID"].includes(o.status))
    .reduce((s, o) => s + o.totalAmount, 0);
  const overdueOrders = orders.filter(
    (o) =>
      o.requiredDeliveryDate &&
      new Date(o.requiredDeliveryDate) < new Date() &&
      !["DELIVERED", "SHIPPED", "PAID", "CLOSED", "CANCELLED"].includes(o.status),
  ).length;
  const shippedMonth = orders.filter(
    (o) =>
      o.status === "SHIPPED" &&
      o.actualDeliveryDate &&
      new Date(o.actualDeliveryDate).getMonth() === new Date().getMonth(),
  ).length;
  ...
  if (orders.length === 0) {
    const em = EMPTY_MESSAGES[lang]?.orders || EMPTY_MESSAGES.vi.orders;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        {[
          ...
          {
            label: t ? "Giá trị Pipeline" : "Pipeline Value",
            value: fmt(pipelineValue),
            icon: DollarSign,
            color: "#10B981",
          },
          {
            label: t ? "Đơn quá hạn" : "Overdue",
            value: overdueOrders,
            icon: AlertTriangle,
            color: overdueOrders > 0 ? "#EF4444" : "#64748B",
          },
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          ...
          <input
            ...
            placeholder={t ? "Tìm đơn hàng..." : "Search orders..."}
            style={{
              width: "100%",
      ...
      >
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {filtered.map((o) => (
                <tr
                  key={o.id}
                  onClick={() => setSelOrder(selOrder?.id === o.id ? null : o)}
                  style={{
                    cursor: "pointer",
                    borderBottom: "1px solid var(--border)",
                    background: selOrder?.id === o.id ? "var(--hover-bg)" : "transparent",
                  }}
                >
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    <Badge color={STATUS_COLORS[o.status] || "#64748B"}>{o.status.replace(/_/g, " ")}</Badge>
                  </td>
                  <td
                    style={{
                      padding: "8px 12px",
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    <Badge color={PAYMENT_COLORS[o.paymentStatus] || "#64748B"}>{o.paymentStatus}</Badge>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {filtered.length === 0 && (
          <div style={{ padding: 30, textAlign: "center", color: "var(--text-dim)" }}>
            {t ? "Không tìm thấy đơn hàng" : "No orders match filters"}
          </div>
        )}
      </div>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 800 }}>
              {selOrder.orderNumber} — {selOrder.customerName}
            </div>
            <button
          ...
          >
            <div>
              <span style={{ color: "var(--text-dim)" }}>PO:</span> {selOrder.poNumber || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Ngày đặt:" : "Order Date:"}</span>{" "}
              {selOrder.orderDate || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Hạn giao:" : "Required:"}</span>{" "}
              {selOrder.requiredDeliveryDate || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Vận chuyển:" : "Shipping:"}</span>{" "}
              {selOrder.shippingMethod || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Tracking:" : "Tracking:"}</span>{" "}
              {selOrder.trackingNumber || "—"}
            </div>
            <div>
              <span style={{ color: "var(--text-dim)" }}>{t ? "Đã thanh toán:" : "Paid:"}</span>{" "}
              {fmt(selOrder.paidAmount)} / {fmt(selOrder.totalAmount)}
            </div>
          </div>
          {selOrder.items.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>
                {t ? "Sản phẩm" : "Line Items"} ({selOrder.items.length})
              </div>
